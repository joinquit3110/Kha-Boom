...
const {error, readFile, success, writeFile, CONFIG, STUDIO_ASSETS, PROJECT_ASSETS, CONTENT, OUTPUT, watchFiles, findFiles, textHash, /* add copy util */} = require('./utilities');
...
async function bundleScripts(srcPath, destPath, minify = false, watch = false, options = {}) {
  // Minimal entry log (keep one lightweight sentinel for troubleshooting)
  if (path.basename(srcPath) === 'course.ts') {
    try { console.log('[KB DEBUG] bundleScripts(course.ts)'); } catch {}
  }
  ...
  for (const locale of options.locales || ['en']) {
    const result = await esbuild.build({
      ...
      metafile: true, // always produce metafile for diagnostics
      write: false,
    ...
    if (path.basename(srcPath) === 'course.ts') {
      // Keep only a concise confirmation that progress component was bundled.
      const inputFiles = Object.keys(result.metafile.inputs || {});
      const hasProgress = inputFiles.some(f => f.endsWith('components/progress/progress.ts'));
      console.log('[KB DEBUG] course.ts build includes progress:', hasProgress);
    }
    ...
    let output = text.replace(/<<([\w\s-:.,]+)>>/g, (_, str) => options.translate?.(locale, str) || str);
  // Remove FORCE_APPEND instrumentation now that diagnostics are pruned.
...
// Helper to copy all project assets (except TS/SCSS) from frontend/assets -> public/
async function copyProjectAssets() {
  const pattern = 'assets/**/*';
  const files = globSync(pattern, {cwd: PROJECT_ASSETS, nodir: true});
  // Determine names of JS bundles produced from top-level TS sources so we don't overwrite them
  // e.g. course.ts -> course.js. Overwriting these with stale legacy copies in frontend/assets
  // was masking new code (progress component + sentinels). We skip those here.
  const builtJsBasenames = new Set();
  try {
    for (const {src} of getAssetFiles('*.ts')) {
      if (src.endsWith('.d.ts')) continue;
      const base = path.basename(src).replace(/\.ts$/, '.js');
      builtJsBasenames.add(base);
    }
  } catch (e) {
    console.log('[KB DEBUG] copyProjectAssets: error determining built JS basenames', e);
  }
  for (const rel of files) {
    // Skip TypeScript/SCSS sources
    if (rel.endsWith('.ts') || rel.endsWith('.scss') || rel.endsWith('.d.ts')) continue;
    // Map assets/icons/* are inputs to sprite; copying originals is optional – skip to avoid clutter
    if (rel.startsWith('assets/icons/')) continue;
    // Prevent overwriting built JS bundles (e.g. course.js) with legacy assets copies.
    const fileName = path.basename(rel);
    if (builtJsBasenames.has(fileName)) {
      console.log(`[KB DEBUG] Skipping project asset that would overwrite built bundle: ${rel}`);
      continue;
    }
    // Explicit guard for historical issue: legacy frontend/assets/course.js overriding built bundle
    if (fileName === 'course.js') {
      console.log('[KB DEBUG] Forced skip of assets/course.js to preserve freshly built course.js bundle');
      continue;
    }
    const src = path.join(PROJECT_ASSETS, rel);
    const outRel = rel.replace(/^assets[\\/]/, '');
    const dest = path.join(OUTPUT, outRel);
    // Ensure directory exists and copy
    await writeFile(dest, fs.readFileSync(src));
  }
  success('frontend/assets -> public', 0);
}
...
async function buildAssets(minify = false, watch = false, locales = ['en']) {
  ...
  // Copy project static assets into public (e.g., /khaboom/**)
  await copyProjectAssets().catch(error('copy assets'));
  ...
  // Diagnostics pruned: intentionally minimal logging retained. Build complete.
}

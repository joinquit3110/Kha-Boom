...
export class Variable extends CustomElementView implements StepComponent {
  ...
  private $valueLabel?: ElementView;
  ...
  ready() {
    ...
    // Simplified transient value label (no animation). Show on change, hide after idle.
    const showValue = () => {
      if (!this.$valueLabel) {
        this.$valueLabel = $N('div', {class: 'var-value-label'}, this) as ElementView;
      }
      this.$valueLabel.text = String(this.value);
      this.$valueLabel.show();
      clearTimeout((this as any)._valueTimer);
      (this as any)._valueTimer = setTimeout(() => this.$valueLabel?.hide(), 1200);
    };
    ...
    // Clicking arrows should also show value briefly
    for (const $a of this.$$('.left, .right')) {
      $a.on('click', () => showValue());
    }
  }
  ...
  if (this.$valueLabel) this.$valueLabel.text = String(this.value);
  }
}

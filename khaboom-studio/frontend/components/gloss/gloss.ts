...
export class Gloss extends CustomElementView {
  ...
  ready() {
    ...
    // Seed a stable but varied accent hue based on the glossary key.
    // This gives each term a distinct, colorful theme without randomness per session.
    let hash = 0;
    for (let i = 0; i < this.xid.length; ++i) hash = (hash * 131 + this.xid.charCodeAt(i)) >>> 0;
    const hue = hash % 360;
    this.css('--gloss-h', String(hue));

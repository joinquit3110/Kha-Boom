...
export class MathigonStudioApp {
  ...
  setup(options: ServerOptions) {
    ...
    this.app.use((req, res, next) => {
      ...
      // Lightweight anonymous progress when accounts are disabled
      if (!CONFIG.accounts.enabled) {
        if (req.cookies.tmp_user) {
    ...
    // Serve dashboard via pug that inlines static `public/khaboom/dashboard.html`
    this.get('/khaboom/dashboard.html', (_req, res) => res.render('dashboard'));

    // JSON list of available courses for dashboard
    this.get('/khaboom/courses.json', (req, res) => {
      try {
        const CATEGORY_KEYWORDS: Record<string, RegExp[]> = {
          'geometry': [/circle|triangle|geometry|poly(gon|hedra)/i],
          'probability': [/probab|statistic|data/i],
          'computer-science': [/code|cipher|algorithm|computer|graph-theory/i],
          'physics': [/fractal|chaos|vectors?|transform|exponential/i],
          'mathematics': [/./]  // fallback
        };
        const inferCategory = (id: string, title: string) => {
          for (const [c, patterns] of Object.entries(CATEGORY_KEYWORDS)) {
            if (patterns.some(p => p.test(id) || p.test(title))) return c;
          }
          return 'mathematics';
        };
        const courses = COURSES.map(id => {
          const course = getCourse(id, req.locale.id) || getCourse(id, 'en');
          const title = (course as any)?.title || id;
            const level = (course as any)?.level || 'All Levels';
          const color = (course as any)?.color;
          const rawCat = (course as any)?.category;
          const category = rawCat || inferCategory(id, title);
          return {id, title, level, color, category};
        });
        res.json(courses);
      } catch (e) {
